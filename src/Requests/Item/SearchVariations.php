 ...
 */
class SearchVariations extends Request
{
	 ...
	 * @param null|int $page Limits the results to a specific page. The page number must be specified. Default value is 1.
	 * @param null|int $itemsPerPage Limits the number of results listed per page to a specific number. The number of variations to be listed per page must be specified. Default value is 50.
	 * @param null|int $id Filter restricts the list of results to variations with the specified variation ID. An variation ID must be specified. More than one ID should be separated by commas.
	 * @param null|int $itemId Filter restricts the list of results to variations with the specified item ID. An item ID must be specified. More than one ID should be separated by commas.
	 * @param null|int $variationTagId Filter restricts the list of results to variations with the specified tag ID. An tag ID must be specified. More than one ID should be separated by commas.
	 * @param null|string $itemName Filter restricts the list of results to variations with the specified item name. An item name must be specified.
	 * @param null|string $flagOne Filter restricts the list of results to variations of items with the flag one. The flag one must be specified.
	 * @param null|string $flagTwo Filter restricts the list of results to variations of items with the flag two. The flag two must be specified.
	 * @param null|int $storeSpecial Filter restricts the list of results to variations of items with the specified store special. The following values are allowed: 0 (None), 1 (Special offer), 2 (New item), 3(Top item).
	 * @param null|int $categoryId Filter restricts the list of results to variations with the specified category id
	 ...
	 * @param null|int $plentyId Filter restricts the list of results to variations that are visible in specified clients. Separate more than one client by commas.
	 * @param null|int $referrerId Filter restricts the list of results to variations that are visible in specified markets. Separate more than one referrer by commas.
	 ...
	 * @param null|string $supplierNumberFuzzy Filter restricts the list of results to variations that contain the specified supplier number (SQL LIKE operator).
	 * @param null|string $sku Filter restricts the list of results to variations with the specified SKU. In additional, results can also be restricted to a specific referrer by specifying the referrer ID after a colon. Example: L0R3MIP5UM:104.1
	 ...
	 * @param null|string $itemDescription Filter restricts the list of results to variations with descriptions that contain the specified string.
	 * @param null|string $stockWarehouseId Filter restricts the list of results to variations which have physical stock on the given warehouse.
	 * @param null|int $supplierId Filter restricts the list of results to variations with the specified supplier ID.
	 */
	public function __construct(
		...
		protected ?int $variationTagId = null,
		protected ?string $itemName = null,
		protected ?string $flagOne = null,
		protected ?string $flagTwo = null,
		protected ?int $storeSpecial = null,
		...
		protected ?string $supplierNumberFuzzy = null,
		...
		protected ?string $stockWarehouseId = null,
